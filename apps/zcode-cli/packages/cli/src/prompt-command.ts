import { wantsJsonSummary, inferAttachmentTypeFromPath, routesToPromptCommandCenter, runPromptCommandCenterCommand, listCustomCommandsForPrompt, resolveHeadlessWorkspaceHookTrustDiagnostic, writeHeadlessWorkspaceHookTrustDiagnostic } from "./prompt-command-support.js";
...
const BACKGROUND_TASKS_RUNNING_ERROR = "Background tasks are still running";

const readPipedPrompt = async (ctx: RunContext): Promise<string> => {
  // A TTY is owned by the interactive prompt. Only an explicitly non-TTY stdin
  // is a prompt source; this avoids blocking ordinary `zcode -p` invocations.
  if (ctx.stdin.isTTY !== false || ctx.stdin.readableEnded) return "";
  const chunks: Buffer[] = [];
...
const mergePipedPrompt = (piped: string, prompt: string): string => {
  const parts = [piped.trim(), prompt.trim()].filter((part) => part.length > 0);
  return parts.join("\n\n");
};
...
export const runPrompt = async (
  ...
  prompt = mergePipedPrompt(await readPipedPrompt(ctx), prompt);
  ...
  let streamsEvents = wantsEventStream(options);
  ...
  try {
    ...
    streamsEvents = wantsEventStream(options);
    ...
    if (options.waitBackground !== false && observer.hasBackgroundActivity() && runtimeFacts) {
      await waitForHeadlessWorkflowSettle({
        runtime: runtimeFacts,
        signal: abortController.signal,
      });
    }
    if (options.waitBackground === false && runtimeFacts?.hasRunningBackgroundTasks()) {
      const taskIds = observer.activeBackgroundTaskIds();
      const suffix = taskIds.length > 0 ? `: ${taskIds.join(", ")}` : ".";
      const errorMessage = `${BACKGROUND_TASKS_RUNNING_ERROR}${suffix}`;
      if (streamsEvents) {
        ctx.stdout.write(
          `${JSON.stringify({
            type: "result",
